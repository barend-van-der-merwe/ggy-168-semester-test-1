...
if image is not None:
    ...
    with col1:
        ...
        chk4 = st.checkbox("Q 11d", key="chk4")

    with col2:
        st.slider("Q 12a", min_value=0.0, max_value=2.0, step=0.5, key="slider1")
        st.slider("Q 12b", min_value=0.0, max_value=2.0, step=0.5, key="slider2")
        st.checkbox("Q 13a", key='chk5')
        st.checkbox("Q 13b", key='chk6')
    ...
    if st.button("Grade"):
        ...
        final_img = cv.putText(img=final_img, text=f'{int(st.session_state.chk1)}', org=(675, 300),
                               fontFace=cv.FONT_HERSHEY_SIMPLEX,
                               fontScale=0.5, color=(0, 0, 255), thickness=1)
        final_img = cv.putText(img=final_img, text=f'{int(st.session_state.chk2)}', org=(675, 340),
                               fontFace=cv.FONT_HERSHEY_SIMPLEX,
                               fontScale=0.5, color=(0, 0, 255), thickness=1)
        final_img = cv.putText(img=final_img, text=f'{int(st.session_state.chk3)}', org=(675, 380),
                               fontFace=cv.FONT_HERSHEY_SIMPLEX,
                               fontScale=0.5, color=(0, 0, 255), thickness=1)
        final_img = cv.putText(img=final_img, text=f'{int(st.session_state.chk4)}', org=(675, 420),
                               fontFace=cv.FONT_HERSHEY_SIMPLEX,
                               fontScale=0.5, color=(0, 0, 255), thickness=1)
        final_img = cv.putText(img=final_img, text=f'{st.session_state.slider1}', org=(675, 440),
                               fontFace=cv.FONT_HERSHEY_SIMPLEX,
                               fontScale=0.5, color=(0, 0, 255), thickness=1)
        final_img = cv.putText(img=final_img, text=f'{st.session_state.slider2}', org=(675, 480),
                               fontFace=cv.FONT_HERSHEY_SIMPLEX,
                               fontScale=0.5, color=(0, 0, 255), thickness=1)
        final_img = cv.putText(img=final_img, text=f'{int(st.session_state.chk5)}', org=(675, 520),
                               fontFace=cv.FONT_HERSHEY_SIMPLEX,
                               fontScale=0.5, color=(0, 0, 255), thickness=1)
        final_img = cv.putText(img=final_img, text=f'{int(st.session_state.chk6)}', org=(675, 560),
                               fontFace=cv.FONT_HERSHEY_SIMPLEX,
                               fontScale=0.5, color=(0, 0, 255), thickness=1)
        st.image(final_img)
