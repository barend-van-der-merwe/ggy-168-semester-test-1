...
if image is not None:
    ...
    st.checkbox("14) True bearing correct (48° - 52°)", key = "chk1")
    st.checkbox("15) Year calculation shown and correct (5 years)", key = "chk2")
    st.checkbox("15) Change in magnetic calculation correct (45'W)", key = "chk3")
    st.checkbox("15) New magnetic declination correct (2.98°W OR 2°59'W)", key = 'chk4')
    st.checkbox("16) Reverse magnetic bearing correct (154° - 163°)", key = "chk5")
    ...
    if st.button("Grade"):
        ...
        final_img = cv.putText(img=final_img, text=f'{int(st.session_state.chk1)}', org=(212,313),
                               fontFace=cv.FONT_HERSHEY_SIMPLEX,
                               fontScale=0.5, color=(0, 0, 255), thickness=1)
        final_img = cv.putText(img=final_img, text=f'{int(st.session_state.chk2)+int(st.session_state.chk3)+int(st.session_state.chk4)}', org=(667,346),
                               fontFace=cv.FONT_HERSHEY_SIMPLEX,
                               fontScale=0.5, color=(0, 0, 255), thickness=1)
        final_img = cv.putText(img=final_img, text=f'{int(st.session_state.chk5)}', org=(212,352),
                               fontFace=cv.FONT_HERSHEY_SIMPLEX,
                               fontScale=0.5, color=(0, 0, 255), thickness=1)
